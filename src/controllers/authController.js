...
const getEndOfDayBangkok = () => {
    ...
    return new Date(bangkokTime.getFullYear(), bangkokTime.getMonth(), bangkokTime.getDate() + 1);
};
...
const login = async (req, res) => {
    ...
    try {
        ...
        res.json({ token, user: { id: user.id, email: user.email, username: user.username, role_id: user.role_id } });
    } catch (error) {
...
const refreshToken = async (req, res) => {
    ...
    try {
        ...
        console.log(`Token refreshed at ${new Date().toISOString()} (UTC), expires at ${endOfDay.toISOString()} (Bangkok time)`);
